...
double Rectangle::Area() const {
    double width = std::sqrt(std::pow(points[0].x - points[1].x, 2) + std::pow(points[0].y - points[1].y, 2));
    double height = std::sqrt(std::pow(points[1].x - points[2].x, 2) + std::pow(points[1].y - points[2].y, 2));
...
std::istream& operator>>(std::istream& is, Rectangle& r) {
    ...
    r = Rectangle(ipoints[0], ipoints[1], ipoints[2], ipoints[3]);
